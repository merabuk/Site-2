...
@section('title', 'Редактировать разрешение')

@section('content_header')
    <h1 class="mr-3 text-dark">Редактировать разрешение</h1>
@stop
                ...
                <form action="{{ route('permissions.update', $permission->id) }}" method="POST">
                    ...
                    @method('PUT')
                        ...
                        <x-adminlte-input name="display_name" error-key="display_name" label="Название"
                            value="{{ old('display_name', $permission->display_name) }}" placeholder="Введите название разрешения"/>
                        {{-- Описание --}}
                        <x-adminlte-input name="description" error-key="description" label="Описание"
                            value="{{ old('description', $permission->description) }}" placeholder="Введите описание группы"/>
                        {{-- Иконка --}}
                        <x-adminlte-input name="icon" error-key="icon" label="Иконка"
                            value="{{ old('icon', $permission->icon) }}" placeholder="Введите код иконки">
                            <x-slot name="prependSlot">
                                <div class="input-group-text">
                                    <i class="{{ old('icon', $permission->icon) }} text-lightblue"></i>
                                </div>
                            </x-slot>
                        </x-adminlte-input>
                        {{-- Группа разрешений --}}
                        ...
                        <x-adminlte-select-bs name="permission_group_id" label="Группа разрешений"
                            ...
                            @forelse ($permissionGroups as $group)
                                <option data-icon="{{ $group->icon }} text-info" data-subtext="{{ $group->name }}" value="{{ $group->id }}"
                                    {{ (collect(old('permissions', (isset($permission->permissionGroup->id) ? $permission->permissionGroup->id : 0) ))->contains($group->id)) ? 'selected' : '' }}>{{ $group->display_name }}</option>
                            @empty
