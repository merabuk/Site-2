...
@section('title', 'Настройки - Редактировать вебхук')

@section('content_header')
    <div class="d-flex justify-content-between">
        <h1 class="mr-3 text-dark">Настройки - Редактировать вебхук</h1>
    </div>
@stop
        ...
        <div class="col-12">
            <x-adminlte-card>
                <form action="{{ route('webhooks.update', $webhook->id) }}" method="POST">
                    ...
                    @method('PUT')
                    {{-- URL --}}
                    <x-adminlte-input name="url" error-key="url" label="URL" value="{{ old('url', $webhook->url) }}"
                        placeholder="Введите URL для отправки" />
                    {{-- Метод --}}
                    <x-adminlte-select2 id="method" name="method" error-key="method" label="Метод запроса">
                        <option value="get" @if (old('method', $webhook->method) == 'get') selected @endif>GET</option>
                        <option value="post" @if (old('method', $webhook->method) == 'post') selected @endif>POST</option>
                    </x-adminlte-select2>
                    {{-- Данные для отправки --}}
                    <x-adminlte-select2 id="data" name="data[]" error-key="data" label="Данные для отправки"
                        ...
                        @forelse ( $params as $param)
                            <option value="{{ $param->code ?? '' }}" @if (in_array($param->code, old('data', $webhook->data))) selected @endif>
                                {{ $param->name ?? '' }}
                            </option>
                        @empty
                        @endforelse
                    ...
                    <div class="custom-control custom-switch custom-switch-off-danger custom-switch-on-success">
                        <input type="checkbox" class="custom-control-input" id="enabled"
                            name="enabled" value="1" @if (old('enabled', $webhook->enabled)) checked @endif>
