...
@section('content_header')
    <h1 class="mr-3 text-dark">Создать группу</h1>
@stop
                ...
                <form action="{{ route('permission-groups.store') }}" method="POST">
                    @csrf
                        ...
                        <x-adminlte-input name="display_name" error-key="display_name" label="Название"
                            value="{{ old('display_name') }}" placeholder="Введите название группы"/>
                        {{-- Описание --}}
                        ...
                        <x-adminlte-select-bs id="permissions" name="permissions[]" label="Разрешения"
                            ...
                            @forelse ($permissions as $permission)
                                <option data-icon="{{ $permission->icon }} text-info" data-subtext="{{ $permission->name }}" value="{{ $permission->id }}"
                                    {{ (collect(old('permissions'))->contains($permission->id)) ? 'selected' : '' }}>{{ $permission->display_name }}</option>
                            @empty
