...
@section('content_header')
    <h1 class="mr-3 text-dark">Редактировать роль</h1>
@stop
                ...
                <form action="{{ route('roles.update', $role->id) }}" method="POST">
                    ...
                    @method('PUT')
                        ...
                        <x-adminlte-input name="display_name" error-key="display_name" label="Название"
                            value="{{ old('display_name', $role->display_name) }}" placeholder="Введите название роли"/>
                        {{-- Описание --}}
                        <x-adminlte-input name="description" error-key="description" label="Описание"
                            value="{{ old('description', $role->description) }}" placeholder="Введите описание группы"/>
                        {{-- Иконка --}}
                        <x-adminlte-input name="icon" error-key="icon" label="Иконка"
                            value="{{ old('icon', $role->icon) }}" placeholder="Введите код иконки">
                            <x-slot name="prependSlot">
                                <div class="input-group-text">
                                    <i class="{{ old('icon', $role->icon) }} text-lightblue"></i>
                                </div>
                            </x-slot>
                        </x-adminlte-input>
                        {{-- Разрешения --}}
                                        ...
                                        <optgroup data-icon="{{ $group->icon }} text-info" label="{{ $group->display_name }}">
                                            @foreach ($group->permissions as $permission)
                                                <option data-icon="{{ $permission->icon }} text-info" data-subtext="{{ $permission->name }}" value="{{ $permission->name }}"
                                                    {{ (collect(old('permissions', $role->permissions()->pluck('name')))->contains($permission->name)) ? 'selected' : '' }}>{{ $permission->display_name }}</option>
                                            @endforeach
                                        </optgroup>
                                    @endif
                                @empty
